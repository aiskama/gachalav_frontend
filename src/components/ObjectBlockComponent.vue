<template>
  <div class="object_block" v-if="data">
    <span v-if="data.title" class="object_block__title">{{ data.title }}</span>
    <div class="object_block__subtitle">
      <span v-if="data.payback"
      >Окупаемость:
        <span
          v-if="data.payback"
          :class="{
            'object_block__subtitle--red': data.payback < 51,
            'object_block__subtitle--yellow': data.payback > 50,
            'object_block__subtitle--green': data.payback > 89,
          }"
        >{{ data.payback }}%</span
        >
      </span>
    ...
    <SliderComponent v-slot="{ item }" :items="data.images" :slider-options="objectSliderOptions">
      <router-link :to="{ name: 'object', params: { id: data.id }  }" class="object_block__img">
